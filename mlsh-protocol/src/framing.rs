//! Length-prefixed JSON framing for QUIC streams.
//!
//! All QUIC-based protocols in mlsh use a 4-byte big-endian length prefix
//! followed by a JSON payload. This module centralises that logic.
...
/// Read a length-prefixed JSON message, returning `None` on clean EOF.
...
/// Read a length-prefixed JSON message. Returns an error on EOF.
